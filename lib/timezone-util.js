...
timezone_util.shiftToTzInfo = function(localTime, toTz) {
...
timezone_util.shiftToTz = function(localTime, toTz) {
 ...
 */
timezone_util.shiftFromTz = function(localtime, fromTz) {
 ...
 */
timezone_util.convertTZtoTZ = function(localTime, tz1, tz2) {
  return this.convertTZtoTZInfo(localTime, tz1, tz2).time;
};

timezone_util.convertTZtoTZInfo = function(localTime, tz1, tz2) {
...
timezone_util.convertOffsetToOffset = function(localTime, offset1, offset2) {
  console.log('convertOffsetToOffset() - ' + localTime);
  console.log('convertOffsetToOffset() - ' + offset1 + ' / ' + offset2);

  var baseOffset = offset2 - offset1;
  console.log('convertOffsetToOffset() - ' + baseOffset + ' (' + (baseOffset / 60) + ')');

  var sign = (0 < offset2) ? 1 : -1;

  var tzTime = new Date(localTime.getTime() + (sign * baseOffset * 60 * 1000));
  console.log('convertOffsetToOffset() - ' + tzTime);

  return tzTime;
};

// JS Dateのタイムゾーンを標準化する（符号を反転する）
timezone_util.canonicalizeJsDateOffseet = function(jsdateoffset) {
  return (-1 * jsdateoffset);
};

module.exports = timezone_util;
