...
timezoneutil.loadAbbrs = function(json) {
  var abbrTz = {};
  ...
  _.each(json.zones, function(v) {
    ...
    for (var i = 0; i < abbrs_length; i++) {
      ...
      // console.log('abbr = ' + abbr + ' / offset + ' + offset + ' / name = ' + name);

      if (!abbrTz[abbr]) {
        abbrTz[abbr] = { offset: [], name: '' };
      }

      if (0 === abbrTz[abbr].offset.length) {
        abbrTz[abbr].offset = [offset];
      } else {
        if (!_.contains(abbrTz[abbr].offset, offset)) {
          abbrTz[abbr].offset.push(offset);
        }
      }

      if (0 === abbrTz[abbr].name.length) {
        abbrTz[abbr].name = [name];
      } else {
        if (!_.contains(abbrTz[abbr].name, name)) {
          abbrTz[abbr].name.push(name);
        }
      }

      // console.log('abbr = ' + abbr + ' / offset + ' + abbrTz[abbr].offset + ' / name = ' + abbrTz[abbr].name + ' / ' + Object.keys(abbrTz).length);
    }
  });

  // console.log(Object.keys(abbrTz).length);

  // var abbrTz2 = _.sortBy(abbrTz, function(v, k) { return k; });
  _.each(abbrTz, function(v, k) {
    console.log(k + ' / offsets = ' + v.offset.join(',') + ' / name = ' + v.name.join(','));
  });
};
